...
async function runAgent(task: string) {
  ...
  while (true) {
    ...
    if (data.stop_reason !== "tool_use") {
      console.log("\n[final]", data.content.find((b: any) => b.type === "text")?.text);
      return;
    }
    const results: any[] = [];
...
runAgent(
  "Implement parseQuery(qs) in query.ts that parses a URL query string into an object " +
  "(handle empty input, repeated keys as arrays, percent-encoded chars). Then write a " +
  "thorough test file query.test.ts. Then review both files and list any bugs you find."
);
